...
struct optiga_pre_handshake_buf {
	...
	uint8_t scratch[OPTIGA_PRE_SCRATCH_LEN];
	size_t scratch_len;
	// TODO(chr): better naming
	uint8_t scratch2[OPTIGA_PRE_SCRATCH_LEN];
	uint8_t rnd[OPTIGA_PRE_RND_LEN];
	uint8_t deriv_secret[OPTIGA_PRE_DERIVED_SECRET_LEN];

	/* Scratch buffers for tls_prf_sha256 */
	uint8_t tmp[OPTIGA_PRE_LABEL_STRLEN + OPTIGA_PRE_SHA256_LEN + OPTIGA_PRE_RND_LEN];
	uint8_t h_i[OPTIGA_PRE_SHA256_LEN];
};

/* Buffer for encrypting and decrypting data after the Handshake */
struct optiga_pre_operation_buf {
	uint8_t encrypted_apdu[OPTIGA_PRE_MAX_ENC_APDU_LEN];
	size_t encrypted_apdu_len;
};

struct present_layer {
	uint8_t pre_shared_secret[OPTIGA_PRE_PRE_SHARED_SECRET_LEN];

	uint8_t master_enc_key[OPTIGA_PRE_AES128_KEY_LEN];
	uint8_t master_dec_key[OPTIGA_PRE_AES128_KEY_LEN];
	uint8_t master_enc_nonce[OPTIGA_PRE_AES128_NONCE_LEN];
	uint8_t master_dec_nonce[OPTIGA_PRE_AES128_NONCE_LEN];

	uint8_t assoc_data_buf[OPTIGA_PRE_ASSOC_DATA_LEN];
...
int optiga_pre_send_apdu(struct device *dev, const uint8_t *data, size_t len);
int optiga_pre_recv_apdu(struct device *dev, uint8_t *data, size_t *len);
